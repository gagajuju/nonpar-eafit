...
```{r, leastsquares2, echo = FALSE, fig.cap = '(ref:leastsquares2title)', screenshot.alt = "images/screenshots/least-squares-3D.png", dev = 'png', cache = TRUE, fig.pos = 'h!', out.width = '70%'}
...
```{r, maximumlikelihood, echo = FALSE, fig.cap = '(ref:maximumlikelihoodtitle)', screenshot.alt = "images/screenshots/log-maximum-likelihood.png", dev = 'png', cache = TRUE, fig.pos = 'h!', out.width = '70%'}
...
```{r, kreg, echo = FALSE, fig.cap = '(ref:kregtitle)', screenshot.alt = "images/screenshots/kreg.png", dev = 'png', cache = TRUE, fig.pos = 'h!', out.width = '70%'}
...
| Support of $Y$ | Distribution | Link $g(x)$ | $g^{-1}(y)$ | $Y\vert(X_1=x_1,\ldots,X_p=x_p)$ |
|:--------------:|:------------:|:-----------:|:-----------:|:-----------------------------:|
| $\mathbb{R}$ | $\mathcal{N}(\mu,\sigma^2)$ | $x$ | $y$ | $\mathcal{N}(\beta_0+\beta_1x_1+\ldots+\beta_px_p,\sigma^2)$|
| $(0,\infty)$ | $\mathrm{Exp}(\lambda)$ | $x^{-1}$ | $y^{-1}$ | $\mathrm{Exp}(\beta_0+\beta_1x_1+\ldots+\beta_px_p)$ |
| $0,1,\ldots,n$ | $\mathrm{B}(n,p)$ | $\log\left(\frac{x}{n-x}\right)$ | $\frac{n}{1-e^{-y}}$ | $\mathrm{B}\left(n,\frac{n}{1-e^{-(\beta_0+\beta_1x_1+\ldots+\beta_px_p)}}\right)$ |
| $0,1,2,\ldots$ | $\mathrm{Pois}(\lambda)$ | $\log(x)$ | $e^y$ | $\mathrm{Pois}(e^{\beta_0+\beta_1x_1+\ldots+\beta_px_p})$ |

The parameter $\theta(x_1,\ldots,x_p)$ of the conditional distribution in the last column can be retrieved with $\theta(x_1,\ldots,x_p)=g(\mathbb{E}[Y|X_1=x_1,\ldots,X_p=x_p])$. Recall that, again, if there is only one predictor, identifying the $j$-th predictor $X_j$ by $X^j$ in the above expressions allows to consider $p$-th order polynomial fits for $g\left(\mathbb{E}[Y|X_1=x_1,\ldots,X_p=x_p]\right)$.
...
```{r, loclik, echo = FALSE, fig.cap = '(ref:locliktitle)', screenshot.alt = "images/screenshots/loclik.png", dev = 'png', cache = TRUE, fig.pos = 'h!', out.width = '70%'}
knitr::include_app('https://ec2-35-177-34-200.eu-west-2.compute.amazonaws.com/loclik/', height = '1000px')
```
...
\begin{align}
\hat m_{\ell}(x;h,p):=&\,g^{-1}\left(\hat\theta(x)\right)\\
=&\,\mathrm{logistic}(\hat\beta_0).(\#eq:glmloc)
\end{align}
...
Bandwidth selection can be done by means of *likelihood cross-validation*. The objective is to maximize the local likelihood fit at $(Y_i,X_i)$ but removing the influence by the datum itself. That is, maximizing
\begin{align}
