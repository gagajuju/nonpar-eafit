...
```{theorem, label = "point"}
Assume that $\int K^{2+\delta}(z)\mathrm{d}z<\infty$^[This is satisfied, for example, if the kernel decreases exponentially, *i.e.*, if $\exists\, \alpha,\,M>0: K(z)\leq e^{-\alpha|z|},\,\forall|z|>M$.] for $\delta>0$. Then, under **A1**--**A3**,
\begin{align}
...
```{exercise}
Using \@ref(eq:kdenorm2) and Exercise \@ref(exr:das), show that $\hat f(x;h)=f(x)+o_\mathbb{P}(1)$.
```
