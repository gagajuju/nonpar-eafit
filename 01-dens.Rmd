...
knitr::include_app('https://ec2-35-177-34-200.eu-west-2.compute.amazonaws.com/kde/', height = '900px')
```
...
\begin{align}
\mathrm{MSE}[\hat f(x;h)]&=\frac{1}{4}\mu^2_2(K)(f''(x))^2+\frac{R(K)}{nh}f(x)+o(h^4+(nh)^{-1}).(\#eq:kdemse)
\end{align}
...
Under **A1**--**A3**,
\begin{align}
\mathrm{MISE}[\hat f(\cdot;h)]&=\frac{1}{4}\mu^2_2(K)R(f'')h^4+\frac{R(K)}{nh}+o(h^4+(nh)^{-1}).(\#eq:kdemise)
\end{align}
...
```
```{proof}
Solving $\frac{\mathrm{d}}{\mathrm{d} h}\mathrm{AMISE}[\hat f(\cdot;h)]=\mu_2^2(K)R(f'')h^3-R(K)n^{-1}h^{-2}=0$ yields $h_\mathrm{AMISE}$ and $\mathrm{AMISE}[\hat f(\cdot;h_\mathrm{AMISE})]$ gives the AMISE-optimal error.
...
$$
\int f''(x)^2\mathrm{d}x=[f''(x)f'(x)]_{-\infty}^{+\infty}-\int f'(x)f'''(x)\mathrm{d}x=-\int f'(x)f'''(x)\mathrm{d}x
$$
...
\begin{align}
\hat\psi_r(g)=\frac{1}{n}\sum_{i=1}^n\hat f^{(r)}(X_i;g)=\frac{1}{n^2}\sum_{i=1}^n\sum_{j=1}^nL_g^{(r)}(X_i-X_j),(\#eq:psir)
\end{align}
...
```{remark}
The derivatives $K^{(r)}$ for the Gaussian kernel can be obtained using *Hermite polynmials*: $\phi^{(r)}(x)=\phi(x)H_r(x)$. For $r=4,6$, $H_4(x)=x^4-6x^2+3$ and $H_6(x)=x^6-16x^4+45x^2-15$.
```
...
\begin{align*}
\mathrm{MISE}[\hat f(\cdot;h)]&=\mathbb{E}\left[\int (\hat f(x;h)-f(x))^2\mathrm{d}x\right]=\mathbb{E}\left[\int \hat f(x;h)^2\mathrm{d}x\right]-2\mathbb{E}\left[\int \hat f(x;h)f(x)\mathrm{d}x\right]\\
...
- $n^{1/10}(\hat h_\mathrm{LSCV}/h_\mathrm{MISE}-1)\stackrel{d}{\longrightarrow}\mathcal{N}(0,\sigma_\mathrm{LSCV}^2)$ and $n^{1/10}(\hat h_\mathrm{BCV}/h_\mathrm{MISE}-1)\stackrel{d}{\longrightarrow}\mathcal{N}(0,\sigma_\mathrm{BCV}^2)$. Both selectors have a slow rate of convergence (compare it with the $n^{1/2}$ of the CLT). Inspection of the variances of both selectors reveals that, for the Gaussian kernel $\sigma_\mathrm{LSCV}^2/\sigma_\mathrm{BCV}^2\approx 15.7$. Therefore, LSCV is considerably more variable than BCV.
...
$$
Densities of this form are specially attractive since they allow for arbitrarily flexibility and, if the Gaussian kernel is employed, they allow for *explicit and exact* MISE expressions:
\begin{align}
\mathrm{MISE}_r[\hat f(\cdot;h)]&=\frac{1}{2\sqrt{\pi}nh}+\mathbf{w}'\{(1-n^{-1})\boldsymbol\Omega_2-2\boldsymbol\Omega_1+\boldsymbol\Omega_0\}\mathbf{w},\notag\\ (\boldsymbol\Omega_a)_{ij}&=\phi_{(ah^2+\sigma_i^2+\sigma_j^2)^{1/2}}(\mu_i-\mu_j),\quad i,j=1,\ldots,r.(\#eq:misenorm)
\end{align}
...
# A Gaussian with the same mean and variance is plotted in dashed lines
par(mfrow = c(2, 2))
plot(MW.nm5)
plot(MW.nm7)
plot(MW.nm10)
...
```{r, kdebwd, echo = FALSE, fig.cap = 'Performance comparison of bandwidth selectors. The RT, DPI, LSCV, and BCV are computed for each sample for a normal mixture density. For each sample, computes the ISEs of the selectors and sorts them from best to worst. Changing the scenarios gives insight on the adequacy of each selector to hard- and simple-to-estimate densities.', screenshot.opts = list(delay = 15), dev = 'png', cache = TRUE, out.width = '90%'}
knitr::include_app('https://ec2-35-177-34-200.eu-west-2.compute.amazonaws.com/kde-bwd/', height = '900px')
```
...
```{exercise}
(practical, $\star\star\star$) Use \@ref(eq:misenorm) and the family of densities of @Marron1992 in `nor1mix` to compare the MISE and AMISE criteria. \@ref(eq:conv) Gaussian kernel (See e.g. Appendix C in @Wand1995) . Present your findings TODO 
