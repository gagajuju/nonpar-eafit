...
Each topic of this contains a mix of theoretical and practical exercises for **grading**. Each student must choose *three exercises* in total (at least one theoretical and other practical) from Sections \@ref(dens-exercises) and \@ref(reg-exercises) and turn them in order to be graded. The grade is weighted according to the difficulty of the exercises, which is given by the number of stars: easy ($\star$), medium ($\star\star$), and hard ($\star\star\star$). The final grade ($0-5$) is 
$$
\frac{1}{3}\sum_{i=1}^n\frac{\mathrm{Score}_i}{5}(2.25+0.75\star_i),
$$
...
f_{X_1\vert \mathbf{X}_{-1}=\mathbf{x}_{-1}}(x_1)&:=\frac{f(\mathbf{x})}{f_{2,\ldots,p}(\mathbf{x}_{-1})}.
\end{align*}
...
- Consider $0\leq r\leq s$. Then $g(x)=x^{r/s}$ is convex and $g(\mathbb{E}[|X|^s])\leq \mathbb{E}[g(|X|^s)]<\mathbb{E}[|X|^r]$. As a consequence $\mathbb{E}[|X|^r]<\infty\implies\mathbb{E}[|X|^s]<\infty$ for $0\leq r\leq s$.
...
\begin{align*}
\mathrm{MSE}[\hat\theta_n]:=&\,\mathbb{E}[(\hat\theta_n-\theta)^2]\\
...
If $X$ is a *degenerate* random variable such that $\mathbb{P}[X=c]=1$, $c\in\mathbb{R}$, then we write $X_n\stackrel{d,\mathbb{P},r,\mathrm{as}}{\longrightarrow} c$ (the list-notation means that the ceonvegence happens for the four types).
...
Given a strictly positive sequence $a_n$ and a sequence of random variables $X_n$,
\begin{align*}
X_n=O_\mathbb{P}(a_n):\iff&\forall\varepsilon>0,\,\exists\,C=C(\varepsilon)>0,\,n_0=n_0(\varepsilon):\\
...
- @Fan1996 (Sections \@ref(reg-kre), \@ref(reg-kreasymp), \@ref(reg-rob), \@ref(reg-pen)).
...
- @Wand1995 (Sections \@ref(dens-kde), \@ref(dens-bwd), \@ref(dens-bwd), \@ref(dens-prac)).
- @Wasserman2004 (Sections \@ref(intro-stoch), \@ref(reg-param)).
- @Wasserman2006 (Sections \@ref(intro-prob), \@ref(reg-loclik), \@ref(reg-pen), \@ref(dens-ci), \@ref(reg-ci), \@ref(reg-pen)).
