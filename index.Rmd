...
# Install packages
install.packages(c("ks", "nor1mix", "KernSmooth", "sm", "rgl"))
```

The codes in the notes assume the packages have been loaded:
...
Each topic of this contains a mix of theoretical and practical exercises for **grading**. Each student must choose *three exercises* in total (at least one theoretical and other practical) from Sections \@ref(dens-exercises) and \@ref(reg-exercises) and turn them in order to be graded. The grade is according to the difficulty of the solved exercises, which is given by the number of stars: easy ($\star$), medium ($\star\star$), and hard ($\star\star\star$). The final grade ($0-10$) is $\frac{1}{3}\sum_{i=1}^n\frac{\mathrm{Score}_i}{10}(5.5+1.5\star_i)$, where $\mathrm{Score}_i$ is the score ($0-10$) for the $i$-th exercise and $\star_i$ represents its number of stars ($1-3$).

## Background and notation {#intro-nota}

We begin by reviewing some elementary results that we will employ during the course, which will also serve to introduce notation and acronyms.

### Basic probability review {#intro-prob}

A tripe $(\Omega,\mathcal{A},\mathbb{P})$ is called a *probability space*. $\Omega$ represents the *sample space*, the set of all possible individual outcomes of a random experiment. $\mathcal{A}$ is a *$\sigma$-field*, a class of subsets of $\Omega$ that is closed under complementation and infinite unions, and such that $\Omega\in\mathcal{A}$. $\mathcal{A}$ represents the collection of possible *events* (combinations of individual outcomes) that are assigned a probability by the *probability measure* $\mathbb{P}$. A *random variable* is a map $X:\Omega\longrightarrow\mathbb{R}$ such that $\{\omega\in\Omega:X(\omega)\leq x\}\in\mathcal{A}$ (the set is *measurable*).
...
$$
\mathbb{E}[g(X)]:=\int g(x)\mathrm{d}F(x)=\begin{cases}\int g(x)f(x)\mathrm{d}x,&\text{if $X$ continuous,}\\\sum_{i} g(x_i)\mathbb{P}[X=x_i],&\text{if $X$ discrete.}\end{cases}
$$
...
We employ bold faces to denote vectors and matrices. A $p$-*random vector* is a map $\mathbf{X}:\Omega\longrightarrow\mathbb{R}^p$, $\mathbf{X}(\omega):=(X_1(\omega),\ldots,X_p(\omega))$, such that each $X_i$ is a random variable. The (joint) cdf of $\mathbf{X}$ is $F(\mathbf{x}):=\mathbb{P}[\mathbf{X}\leq \mathbf{x}]:=\mathbb{P}[X_1\leq x_1,\ldots,X_p\leq x_p]$ and, if $\mathbf{X}$ is continuous, its (joint) pdf is $f:=\frac{\partial^p}{\partial x_1\cdots\partial x_p}F$. The *marginals* of $F$ and $f$ are the cdf and pdf of $X_i$, $i=1,\ldots,p$, respectively. They are defined as:
\begin{align*}
F_{X_i}(x_i)&:=\mathbb{P}[X_i\leq x]=\int_{\mathbb{R}^{p-1}} F(\mathbf{x})\mathrm{d}x_1\cdots\mathrm{d}\check{x_i}\cdots\mathrm{d}x_p,\\
f_{X_i}(x_i)&:=\frac{\partial}{\partial x_i}F_{X_i}(x_i)=\int_{\mathbb{R}^{p-1}} f(\mathbf{x})\mathrm{d}x_1\cdots\mathrm{d}\check{x_i}\cdots\mathrm{d}x_p,
\end{align*} 
where $\check{x_i}$ means that $x_i$ is omitted. The definitions can be extended analogously to the marginals of the cdf and pdf of different subsets of $\mathbf{X}$.

The *conditional* cdf and pdf of $X_1\vert(X_2,\ldots,X_p)$ are defined, respectively, as
\begin{align*}
F_{X_1\vert X_2=x_2,\ldots,X_p=x_p}(x_1)&:=\mathbb{P}[X_1\leq x_1\vert X_2=x_2,\ldots,X_p=x_p],\\
f_{X_1\vert X_2=x_2,\ldots,X_p=x_p}(x_1)&:=\frac{f(\mathbf{x})}{f_{2,\ldots,p}(x_2,\ldots,x_p)}.
\end{align*}
...
$$
The *conditional variance* of $Y|X$ is defined as $\mathbb{V}\mathrm{ar}[Y\vert X]:=\mathbb{E}[(Y-\mathbb{E}[Y\vert X])^2\vert X]=\mathbb{E}[Y^2\vert X]-\mathbb{E}[Y\vert X]^2$.
...
```{example}
Jensen's inequality has interesting derivarions. For example:
...
- Consider $0\leq r\leq s$. Then $g(x)=x^\frac{r}{s}$ is convex and $g(\mathbb{E}[|X|^s])\leq \mathbb{E}[g(|X|^s)]<\mathbb{E}[|X|^r]$. As a consequence $\mathbb{E}[|X|^r]<\infty\implies\mathbb{E}[|X|^s]<\infty$ for $0\leq r\leq s$.
...
- $\mathcal{N}(\mu,\sigma^2)$ stands for the *normal distribution* with mean $\mu$ and variance $\sigma^2$. Its pdf is $\phi_\sigma(x-\mu):=\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(x-\mu)^2}{2\sigma^2}}$, $x\in\mathbb{R}$, and satisfies that $\phi_\sigma(x-\mu)=\frac{1}{\sigma}\phi_1\left(\frac{x-\mu}{\sigma}\right)$. Its cdf is denoted as $\Phi_\sigma(x-\mu)$. $z_\alpha$ denotes the upper $\alpha$-quantile of a $\mathcal{N}(0,1)$, *i.e.*, $z_\alpha=\Phi_1^{-1}(1-\alpha)$. Some uncentered moments of $X\sim\mathcal{N}(\mu,\sigma^2)$ are
$$
\mathbb{E}[X]=\mu,\;\mathbb{E}[X^2]=\mu^2+\sigma^2,\; \mathbb{E}[X^3]=\mu^3+3\mu\sigma^2,\; \mathbb{E}[X^4]=\mu^4+6\mu^2\sigma^2+3\sigma^4.
$$
- The *exponential distribution* is denoted as $\mathrm{Exp}(\lambda)$ and has pdf $f(x;\lambda)=\lambda e^{-\lambda x}$, $\lambda,x>0$.
- The *gamma distribution* is denoted as $\Gamma(a,p)$ and has pdf $f(x;a,p)=\frac{a^p}{\Gamma(p)} x^{p-1}e^{-a x}$, $a,p,x>0$, where $\Gamma(p)=\int_0^\infty x^{p-1}e^{-ax}\mathrm{d}x$. It is known that
$$
\mathbb{E}[\Gamma(a,p)]=\frac{p}{a},\;\mathbb{V}\mathrm{ar}[\Gamma(a,p)]=\frac{p}{a^2}.
$$
...
$$
\mathbb{E}[\mathrm{IG}(a,p)]=\frac{a}{p-1},\;\mathbb{V}\mathrm{ar}[\mathrm{IG}(a,p)]=\frac{a^2}{(p-1)^2(p-2)}.
$$
- The *binomial distribution* is denoted as $\mathrm{B}(n,p)$. Recall that $\mathbb{E}[\mathrm{B}(n,p)]=np$ and $\mathbb{V}\mathrm{ar}[\mathrm{B}(n,p)]=np(1-p)$.
- The *beta distribution* is denoted as $\beta(a,b)$ and its pdf is $f(x;a,b)=\frac{1}{\beta(a,b)}x^{a-1}(1-x)^{1-b}$, where $\beta(a,b)=\frac{\Gamma(a)\Gamma(b)}{\Gamma(a+b)}$. When $a=b=1$, the *uniform distribution* $\mathcal{U}(0,1)$ arises.
...
$X_n$ *converges almost surely* (as) to $X$, written $X_n\stackrel{\mathrm{as}}{\longrightarrow}X$, if $\mathbb{P}[\{\omega\in\Omega:\lim_{n\to\infty}X_n(s)=X(s)\}]=1$.
```
...
$$
\mathrm{MSE}[\hat\theta_n]:=\mathbb{E}[(\hat\theta_n-\theta)^2]=(\mathbb{E}[\hat\theta_n]-\theta)^2+\mathbb{V}\mathrm{ar}[\hat\theta_n]=:\mathrm{Bias}[\hat\theta_n]^2+\mathbb{V}\mathrm{ar}[\hat\theta_n],
$$
goes to zero as $n\to\infty$. Equivalently, if $\hat\theta_n\stackrel{2}{\longrightarrow}\theta$.

If $X$ is a *degenerate* random variable such that $\mathbb{P}[X=c]=1$, $c\in\mathbb{R}$, then we write $X_n\stackrel{d,\mathbb{P},r,\mathrm{as}}{\longrightarrow} c$ (the notation applies to the four types of convergences).
...
iv. If $S_n=\sum_{i=1}^nX_i$ with $X_1,\ldots,X_n$ iid, then $S_n\stackrel{r}{\longrightarrow}S\iff S_n\stackrel{\mathrm{as}}{\longrightarrow}S$.

Also, if $s>r\geq 1$, then $X_n\stackrel{s}{\longrightarrow}X\implies X_n\stackrel{r}{\longrightarrow}X$.
```
...
Let $X_n$ be a sequence of iid random variables with $\mathbb{E}[X_n]=\mu$ and $\mathbb{V}\mathrm{ar}[X_n]=\sigma^2<\infty$, $i\in\mathbb{N}$. Then:
$$
...
Finally, the next results will be of usefulness.
...
```{remark}
Recall the abscense of results for convergence in distribution. They do not hold. Particularly: $X_n\stackrel{d}{\longrightarrow}X$ and $Y_n\stackrel{d}{\longrightarrow}X$ $\,\,\,\not\!\!\!\!\implies X_n+Y_n\stackrel{d}{\longrightarrow}X+Y$.
```
...
```{example, label = "ml"}
It is well known that, given a parametric density $f_\theta$, with parameter $\theta\in\Theta$, and iid $X_1,\ldots,X_n\sim f_\theta$, then the *maximum likelihood* estimator $\hat\theta_{\mathrm{ML}}:=\arg\max_{\theta\in\Theta}\sum_{i=1}^n\log f_\theta(X_i)$ converges to a normal under certain regularity conditions:
$$
...
### Review on $O_\mathbb{P}$ and $o_\mathbb{P}$ notation {#intro-ohs}
...
i. $O(ka_n)=O(a_n)$, $o(ka_n)=o(a_n)$, $k\in\mathbb{R}$.
...
$$
|a+b|^p\leq C_p(|a|^p+|b|^p), \quad C_p=\begin{cases}1,&0<p\leq1,\\2^{p-1},&p>1.\end{cases}
$$
```
...
\iff& \lim_{n\to\infty}\mathbb{P}\left[\frac{|X_n|}{a_n}>\varepsilon\right]=0,\quad\forall\varepsilon>0.
\end{align*}
If $X_n=o_\mathbb{P}(a_n)$, then we say that *$X_n$ is little-$o_\mathbb{P}$ of $a_n$*.
```
...
```{example}
Let $Y_n=o_\mathbb{P}\left(n^{-\frac{1}{2}}\right)$ and $Z_n=o_\mathbb{P}\left(n^{-1}\right)$. Then $Z_n$ converges faster to zero in probability than $Y_n$. To visualize this, recall that the little-$o_\mathbb{P}$ and limit definitions entail that
$$
\forall\varepsilon,\delta>0,\,\exists \,n_0=n_0(\varepsilon,\delta):\forall n\geq n_0,\, \mathbb{P}\left[|X_n|>a_n\varepsilon\right]<\delta.
$$
Therefore, for fixed $\varepsilon,\delta>0$, and a fixed $n\geq\max(n_{0,Y},n_{0,Z})$, then  $\mathbb{P}\left[Y_n\in\left(-n^{-\frac{1}{2}}\varepsilon,n^{-\frac{1}{2}}\varepsilon\right)\right]<1-\delta$ and
$\mathbb{P}\left[Z_n\in(-n^{-1}\varepsilon,n^{-1}\varepsilon)\right]<1-\delta$, but the latter interval is much shorter, hence $Z_n$ is more tighly concentrated around $0$.
```

```{definition, name = "Big $O_\\mathbb{P}$"}
Given a strictly positive sequence $a_n$ and a sequence of random variables $X_n$,
\begin{align*}
X_n=O_\mathbb{P}(a_n):\iff&\forall\varepsilon>0,\,\exists\,C=C(\varepsilon)>0,\,n_0=n_0(\varepsilon):\forall n\geq n_0,\, \mathbb{P}\left[\frac{|X_n|}{a_n}>C_\varepsilon\right]<\varepsilon\\
...
$$
X_n-c=X_n-c_n+c_n-c=\frac{1}{a_n}a_n(X_n-c_n)+o(1)=\frac{1}{a_n}O_\mathbb{P}(1)+o(1).
$$
```
...
$$
From the squared bias and variance, we can get the Mean Squared Error (MSE):
$$
\mathrm{MSE}[F_n(x)]=(\mathbb{E}[F_n(x)]-F(x))^2+\mathbb{V}\mathrm{ar}[F_n(x)]=\frac{F(x)(1-F(x))}{n}.
$$
...
- @Scott2015 (Sections \@ref(dens-hist), \@ref(dens-bwd), \@ref(dens-kde)--\@ref(dens-bwd)))
- @Silverman1986 (Sections \@ref(dens-kde), \@ref(dens-bwd))
- @Vaart1998 (Sections \@ref(intro-stoch), \@ref(intro-ohs))
- @Wand1995 (Sections \@ref(dens-kde), \@ref(dens-bwd), \@ref(dens-kde)--\@ref(dens-bwd)))
- @Wasserman2004 (Sections \@ref(intro-stoch))
- @Wasserman2006 (Sections \@ref(intro-prob), \@ref(dens-loclik), \@ref(reg-pen), \@ref(dens-ci))
- @Wood2006 (Sections )

In addition, these notes are possible due to the existence of the 
@R-bookdown, @R-knitr, @R-rmarkdown, and @R-base.
