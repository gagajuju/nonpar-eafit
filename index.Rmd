...
$$
where $\mathrm{Score}_i$ is the score ($0-5$) for the $i$-th exercise and $\star_i$ represents its number of stars ($1-3$). Deadline for submission is 2017-09-10.
...
The $2$-mean convergence plays a remarkable role in defining a consistent estimator $\hat\theta=T(X_1,\ldots,X_n)$ of a parameter $\theta$. We say that the *estimator is consistent* if its *mean squared error* (MSE),
\begin{align*}
\mathrm{MSE}[\hat\theta]:=&\,\mathbb{E}[(\hat\theta-\theta)^2]\\
=&\,(\mathbb{E}[\hat\theta]-\theta)^2+\mathbb{V}\mathrm{ar}[\hat\theta]\\
=:&\mathrm{Bias}[\hat\theta]^2+\mathbb{V}\mathrm{ar}[\hat\theta],
\end{align*}
...
- @Fan1996 (Sections \@ref(reg-kre), \@ref(reg-asymp), \@ref(reg-rob), \@ref(reg-pen)).
...
- @Wand1995 (Sections \@ref(dens-kde), \@ref(dens-bwd), \@ref(dens-prac), \@ref(dens-mult)).
- @Wasserman2004 (Sections \@ref(intro-stoch), \@ref(reg-param)).
- @Wasserman2006 (Sections \@ref(intro-prob), \@ref(reg-loclik), \@ref(reg-pen), \@ref(dens-ci), \@ref(reg-ci)).
